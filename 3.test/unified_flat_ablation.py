# correct label
#python3 scripts/UMBC/3.test/unified_flat_ablation.py
# incorrect label
#python3 scripts/UMBC/3.test/unified_flat_ablation.py --terrain_id 1
...
parser.add_argument("--terrain_id", type=int, default=None,
                    help="Override the terrain ID. If not set, defaults to the 'flat' mapping from JSON.")
...
class H1FlatDemo:
    def __init__(self):
        ...
        self.policy.load_state_dict(torch.load('scripts/UMBC/logs/models/unified_policy.pth', weights_only=False))
        self.policy.eval()
        self.create_camera()
...
def main():
    ...
    if args_cli.terrain_id is not None:
        with open("scripts/UMBC/logs/metrics/unified_flat_ablation_wrong_label.pkl", "wb") as f:
            pickle.dump(results, f)
    else:
        with open("scripts/UMBC/logs/metrics/unified_flat_ablation.pkl", "wb") as f:
            pickle.dump(results, f)

    print('===============')
    print(f'Unified on flat terrain (Terrain ID: {terrain_id})')
    print('===============')
