...
"""
This script demonstrates an interactive demo with the H1 flat terrain environment.

.. code-block:: bash

    # Usage
    ./isaaclab.sh -p UMBC/data_collection/stairway/phase3.py
...
class H1FlatDemo:
    """This class provides an interactive demo for the H1 flat terrain environment.
    It loads a pre-trained checkpoint for the Isaac-Velocity-Flat-H1-v0 task, trained with RSL RL
    ...
    * ESC: exit current third-person view"""
    ...
    def __init__(self):
        ...
        checkpoint = "/home/whitebot/Workspace/Research/IsaacLab/logs/rsl_rl/h1_stairway/2025-12-12_02-09-27/model_2999.pt"
        # create envionrment
...
def main():
    ...
    action_df = {}
    for i in range(1,20):
        action_df[f'motor_{i}'] = []
    action_df = pd.DataFrame(action_df)
    
    demo_h1 = H1FlatDemo()
    obs, _ = demo_h1.env.reset()
    for i in obs: obs_df.loc[len(obs_df)] = i.tolist()
    for t in range(2800):
        print(t)
    #while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            for i in action: action_df.loc[len(action_df)] = i.tolist()
            obs, _, _, _ = demo_h1.env.step(action)
            for i in obs: obs_df.loc[len(obs_df)] = i.tolist()
            # overwrite command based on keyboard input
            #obs[:, 9:13] = demo_h1.commands

    action_df.to_csv('UMBC/logs/dataset/stairway/3_action_df.csv', index=False)
    obs_df.to_csv('UMBC/logs/dataset/stairway/3_obs_df.csv', index=False)
