...
.. code-block:: bash
    ...
    # Usage
    ./isaaclab.sh -p UMBC/data_collection/stairway/phase1.py
...
def main():
    ...
    obs_data = []
    for obs in obs_list:
        obs_data.extend(obs.tolist())
        
    action_data = []
    for action in action_list:
        action_data.extend(action.tolist())
        
    obs_df = pd.DataFrame(obs_data, columns=[f'obs_{i+1}' for i in range(256)])
    action_df = pd.DataFrame(action_data, columns=[f'motor_{i+1}' for i in range(19)])
    
    print(f"Processing took {time.time() - start_time} seconds.")

    # saving data
    action_df.to_csv('UMBC/logs/dataset/stairway/1_action_df.csv', index=False)
    obs_df.to_csv('UMBC/logs/dataset/stairway/1_obs_df.csv', index=False)
