...
.. code-block:: bash
    ...
    # Usage
    ./isaaclab.sh -p UMBC/data_collection/flat/phase2.py
...
class H1FlatDemo:
    ...
    * ESC: exit current third-person view"""
    ...
    def __init__(self):
        ...
        checkpoint = "UMBC/logs/policy/flat_expert.pt"
        # create envionrment
        env_cfg = H1FlatEnvCfg_PLAY()
        env_cfg.scene.num_envs = 25
        env_cfg.curriculum = None
        # 1. Set forward speed (X) to your desired range
        env_cfg.commands.base_velocity.ranges.lin_vel_x = (0.0, 0.5) 
        ...
        env_cfg.commands.base_velocity.ranges.heading = (0.1, 0.5)
...
def main():
    ...
    action_df.to_csv('UMBC/logs/dataset/flat/2_action_df.csv', index=False)
    obs_df.to_csv('UMBC/logs/dataset/flat/2_obs_df.csv', index=False)
