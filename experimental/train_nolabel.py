'''
python3 UMBC/experimental/train_nolabel.py
'''
...
def retrain_and_log(study_name, run_name, params, dataset, save_path):
    ...
    # save model
    model_path = f"UMBC/logs/policy/no_label_UMBC{save_path}.pth"
    ...
    plot_path = f"UMBC/logs/imgs/nolabel_{save_path}_overall_loss.png"
    ...
    plt.legend(); plt.grid(True)
    plot_path = f"UMBC/logs/imgs/nolabel_{save_path}_terrain_breakdown.png"
    plt.savefig(plot_path)
    plt.close()
    ...
    plt.legend(); plt.grid(True)
    plot_path = f"UMBC/logs/imgs/nolabel_{save_path}_mae_precision.png"
    plt.savefig(plot_path)
    plt.close()
