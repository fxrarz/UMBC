# python3 scripts/UMBC/2.unified_policy/multi_seed_train.py
...
# -----------------------
def retrain_and_log(params, terrain_dirs, save_path, current_seed):
    ...
    traj_train, traj_test = train_test_split(
        manifest, test_size=0.1, stratify=manifest["terrain"], random_state=current_seed
    )
    ...
    # Save terrain mapping specifically for this seed
    with open(f"./scripts/UMBC/logs/models/{save_path}_terrain_mapping.json", "w") as f:
    ...
    history = {'train_loss': [], 'val_loss': [], 'train_mae': [], 'val_mae': [], 'val_rmse': []}
...
if __name__ == '__main__':
    ...
    # Define standard seeds for replication in papers
    seeds = [42, 123, 456, 789, 999]
    all_results = []

    for seed in seeds:
        print("\n" + "="*80)
        print(f"RUNNING TRAINING PIPELINE FOR SEED: {seed}")
        print("="*80)
        
        # 1. Reset all random generators to guarantee isolation between runs
        set_global_seed(seed)
        
        # 2. Assign unique save paths per seed to prevent overwrite
        save_path = f"unified_policy_seed_{seed}"
        
        # 3. Train
        final_val_loss, final_mae, final_rmse = retrain_and_log(
            best_params, 
            terrain_dirs, 
            save_path,
            current_seed=seed
    ...
    # ---------------------------------------------------------
    # Final Paper Summary Aggregation
    # ---------------------------------------------------------
    ...
    mean_loss, std_loss = df_results['Val Loss'].mean(), df_results['Val Loss'].std()
    mean_mae, std_mae = df_results['MAE'].mean(), df_results['MAE'].std()
    mean_rmse, std_rmse = df_results['RMSE'].mean(), df_results['RMSE'].std()
    
    # Fill in formatting if std is NaN (which happens if there's only 1 seed)
    std_loss = std_loss if pd.notna(std_loss) else 0.0
    std_mae = std_mae if pd.notna(std_mae) else 0.0
    std_rmse = std_rmse if pd.notna(std_rmse) else 0.0

    print(f"Mean Val Loss: {mean_loss:.6f} ± {std_loss:.6f}")
    print(f"Mean MAE:      {mean_mae:.6f} ± {std_mae:.6f}")
